...
extern int g_name_len;                   // length of player's name
extern int g_num_to_grow;                // number of units left to grow
extern int g_food_growing_unit;          // number of units growing on each food
...
//   int location; // cell position of the snake's body on the board
//   struct snake_node* next; // point to next location of the snake's body
//   struct snake_node* prev; // point to previous location of the snake's body
// } snake_node; 

// a list stores the value of the snake's body nodes
typedef struct snake {
    // TODO: Define your snake struct! (in ')
    // Store any necessary information about your snake here.
    node_t* head;
    enum input_key direction; // Current direction of the snake's movement
} snake_t;
